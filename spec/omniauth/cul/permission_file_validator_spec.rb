...
RSpec.describe Omniauth::Cul::PermissionFileValidator do
  ...
  describe ".permitted?" do
    ...
    it "returns false for a user who has not been explicitly permitted, and who has no explicitly permitted affiliations" do
      expect(described_class.permitted?(other_user_id, empty_affils)).to eq(false)
    end

    it "returns true for a user who has not been explicitly permitted, but who does have an explicitly permitted affiliation" do
      expect(described_class.permitted?(other_user_id, empty_affils)).to eq(false)
    end
  end
end
